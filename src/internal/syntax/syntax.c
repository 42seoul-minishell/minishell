...
static int	_set_syntax_error(t_doubly_node *node)
{
	if (node->token->type == BRACKET)
		syntax_error(&node->token->value[0], 1);
	else
		syntax_error(node->token->value, 0);
	return (FALSE);
}
...
static int	_is_valid_tokens(t_doubly_list *lst)
{
	...
	while (cur_node != lst->header.prev)
	{
		...
		if (_is_valid_next_token(cur_node, next_node) == FALSE)
			return (_set_syntax_error(cur_node));
